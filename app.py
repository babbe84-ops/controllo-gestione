...
if sezione == "📈 Dashboard Grafica":
    ...
    with t6:
        ...
        st.markdown("*Clicca su una riga della tabella per visualizzare il dettaglio mensile del dipendente sotto.*")
        ...
        dip_scelto = "D'ALSAZIA"
        if event_dip and "selection" in event_dip and event_dip["selection"]["rows"]:
            row_idx = event_dip["selection"]["rows"][0]
            dip_selezionato_riga = df_riassunto_2026.iloc[row_idx]["COGNOME"]
            if dip_selezionato_riga in dati_dipendenti_mensili:
                dip_scelto = dip_selezionato_riga

        st.markdown("---")
        st.subheader(f"🔎 Dettaglio Mensile Dipendente: **{dip_scelto}**")
        ...
        with col_p1:
            ...
            if selected_pie and "selection" in selected_pie:
                ...
                if points:
                    ...
                    if cli_cliccato in dati_clienti_mensili and st.session_state["cliente_selezionato"] != cli_cliccato:
